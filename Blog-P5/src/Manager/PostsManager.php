...
class PostsManager
{
	...
	{
		$request = $this->db->db->query('SELECT * FROM posts ORDER BY id DESC')->fetchAll();
		$postsArray = [];
	...
	{
		$request = $this->db->db->query("SELECT * FROM comments WHERE post_id=$idPost ORDER BY id DESC")->fetchAll();
		$commentsArray = [];
